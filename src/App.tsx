...
const LoadingScreen = ({ onComplete }: { onComplete?: () => void }) => {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      ...
      // 5. Parallax reveal the main content with a deep 3D scale and unfold perspective
    ...
    <>
      ...
      <Suspense fallback={null}>
        {currentPath === '/' && <FloatingDoodles />}
            ...
            {notification && (
              <div
                className={`fixed top-20 right-6 md:right-12 z-200 border-2 border-obsidian px-5 py-3.5 rounded-xl flex items-center gap-3 brutalist-shadow text-xs sm:text-sm font-tag font-bold animate-slide-up-fade ${
                  notification.type === 'error'
                    ? 'bg-[#FEE2E2] text-[#991B1B]'
                    : 'bg-[#E8FBF0] text-[#166534]'
                }`}
              >
                <div className={`w-6 h-6 rounded-full text-white flex items-center justify-center font-black border border-obsidian ${
